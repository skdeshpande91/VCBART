//
//  vc_bart_ind.cpp
//  A new version meant for independent errors
//
...
// [[Rcpp::export]]
Rcpp::List vc_BART_ind(arma::vec Y, // n_train x 1 ... concatenation of all observed Y's
...
{
  if(verbose == true) Rcpp::Rcout << "Entering VC-BART with independent errors" << std::endl;
  Rcpp::RNGScope scope;
  RNG gen;
  ...
  if(verbose == true) Rcpp::Rcout << "[vc-bart-ind]: Entering MCMC" << std::endl;
  time_t tp;
  ...
  for(size_t iter = 0; iter < nd+burn; iter++){
    ...
    for(size_t k = 0; k < p; k++){
      ...
      for(size_t t = 0; t < m; t++){
        ...
        bd_ind(t_vec[k][t], sigma, xi, di, b_tree_pi, gen);
  ...
  if(verbose == true) Rcpp::Rcout << "[vc-bart-ind]: Finished MCMC! time = " << time2 - time1;
  delete[] y_ptr;
  delete[] x_ptr;
  delete[] x_pred_ptr;
  delete[] z_ptr;
