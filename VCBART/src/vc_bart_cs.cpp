//
//  vc_bart_cs.cpp
//    A new version meant for compound symmetry error structure
//
//  Created by Sameer Deshpande on 4 February 2020
//
...
// [[Rcpp::export]]
Rcpp::List vc_BART_cs(arma::vec Y, // n_train x 1 ... concatenation of all observed Y's
...
{
  if(verbose == true) Rcpp::Rcout << "Entering VC-BART w/ compounded symmetry error" << std::endl;
  Rcpp::RNGScope scope;
  RNG gen;
  ...
  // from now on we will not use n_vec, start_index_vec, end_index_vec, or rho_vec
  ...
  double* y_ptr = new double[n_obs_train];
  double* x_ptr = new double[n_obs_train * p];
  double* z_ptr = new double[n_obs_train * R];
  
  double* x_pred_ptr = new double[n_obs_test * p];
  double* z_pred_ptr = new double[n_obs_test * R];
  ...
  b_tree_pi.nu.clear();
  b_tree_pi.nu.resize(p, nu_sigma_mu) ; // use a half-t_7 distribution centered at A
  
  double* sigma_mu = new double[p]; //
  //for(size_t k = 0; k < p; k++) sigma_mu[k] = 1.0/sqrt( (double) m); // 27 August 2019 -- this implies beta_k(z) ~ N(0,1) for all k,z a priori
  for(size_t k = 0; k < p; k++) sigma_mu[k] = sigma_mu_vec(k); // 4 November 2019 -- sigma_mu is now being passed in as an argument
  b_tree_pi.sigma_mu = &sigma_mu[0];
  ...
  arma::mat sigma_mu_samples(p, nd+burn);
  
  arma::vec rho_samples(nd + burn); // save all of the rho samples
  
  double tmp_int = 0.0; // holds value of the offset
  double tmp_fit = 0.0; // holds individual prediction for E[y_ij]
  
  if(verbose == true) Rcpp::Rcout << "[vc-bart-cs]: Entering MCMC" << std::endl;
  time_t tp;
  size_t time1 = time(&tp);
  ...
  for(size_t iter = 0; iter < nd+burn; iter++){
    ...
    for(size_t k = 0; k < p; k++){
      ...
      for(size_t t = 0; t < m; t++){
        fit(t_vec[k][t], xi, di, ftemp); // get fit of tree
        for(size_t i = 0; i < N_train; i++){
          for(size_t j = 0; j < n_train[i]; j++){
        ...
        bd_cs(t_vec[k][t], rho, sigma, xi, di, b_tree_pi, gen);
        
        fit(t_vec[k][t], xi, di, ftemp);
        for(size_t i = 0; i < N_train; i++){
          for(size_t j = 0; j < n_train[i]; j++){
    ...
    //Rcpp::Rcout << "After tree update r_full[0] = " << r_full[0] << std::endl;
    
    // update sigma
    //Rcpp::Rcout << "  Before update sigma = " << sigma << std::endl;
    
    if(ht_sigma_mu == true) update_sigma_mu(sigma_mu, t_vec, b_tree_pi, gen);
    
    if(ht_sigma_y == true) update_sigma_ht_cs(sigma, rho, sigma_pi, di, gen);
    else update_sigma_ig_cs(sigma, rho, sigma_pi, di, gen);
    
    //update_sigma_ig(sigma, rho, sigma_pi, di, gen);
    //Rcpp::Rcout << "  After update sigma = " << sigma << std::endl;
    // save sigma samples
    for(size_t k = 0; k < p;k++) sigma_mu_samples(k, iter) = sigma_mu[k];
    ...
    if(iter >= burn){
      ...
      for(size_t k = 0; k < p; k++){
        ...
        for(size_t t = 0; t < m; t++){
          fit(t_vec[k][t], xi, dip, ftemp_pred);
          for(size_t i = 0; i < N_test; i++){
  ...
  size_t time2 = time(&tp);
  if(verbose == true) Rcpp::Rcout << "[vc-bart-cs]: Finished MCMC! time = " << time2 - time1;
  delete[] y_ptr;
  delete[] x_ptr;
  delete[] x_pred_ptr;
  delete[] z_ptr;
