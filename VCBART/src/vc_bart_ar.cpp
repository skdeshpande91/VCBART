//
//  vc_bart_ar.cpp
//    A new version meant for AR(1) error structure
//
...
// [[Rcpp::export]]
Rcpp::List vc_BART_ar(arma::vec Y, // n_train x 1 ... concatenation of all observed Y's
...
{
  if(verbose == true) Rcpp::Rcout << "Entering VC-BART w/ AR(1) error" << std::endl;
  Rcpp::RNGScope scope;
  RNG gen;
  ...
  if(verbose == true) Rcpp::Rcout << "[vc-bart-ar]: Starting MCMC" << std::endl;
  time_t tp;
  ...
  for(size_t iter = 0; iter < nd+burn; iter++){
    ...
    for(size_t k = 0; k < p; k++){
      ...
      for(size_t t = 0; t < m; t++){
        ...
        bd_ar(t_vec[k][t], rho, sigma, xi, di, b_tree_pi, gen);
    ...
    if(ht_sigma_y == true) update_sigma_ht_ar(sigma, rho, sigma_pi, di, gen);
    else update_sigma_ig_ar(sigma, rho, sigma_pi, di, gen);
  ...
  if(verbose == true) Rcpp::Rcout << "[vc-bart-ar]: Finished MCMC! time = " << time2 - time1;
  delete[] y_ptr;
  delete[] x_ptr;
  delete[] x_pred_ptr;
  delete[] z_ptr;
