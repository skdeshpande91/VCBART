...
tree::tree_cp tree::get_bn(double* z_cont, int* z_cat)
{
  ...
  if(rule.is_aa && !rule.is_cat){
    ...
    if(z_cat != 0){
      ...
      r_count = rule.r_vals.count(z_cat[rule.v_cat]);
      
      if(l_count == 1 && r_count == 0) return l->get_bn(z_cont, z_cat);
      else if(l_count == 0 && r_count == 1) return r->get_bn(z_cont, z_cat);
...
double tree::evaluate(double* z_cont, int* z_cat)
{
  tree::tree_cp bn = get_bn(z_cont, z_cat);
  ...
  if(p){
    if(p->rule.is_aa && p->rule.v_aa == v){
      ...
      if(this == p->l){
        ...
        // this's parent splits on v so any observation in this should be less than p->rule.c
