# p3R2 simulations
# Be sure to run the script "generate_p3R2_data.R" before running this
...
colnames(tmp_data_all) <- c("Y", "X1", "X2", "X3", "Z1", "Z2")

# In this example Z2 is a categorical variable.
# To ensure we use an appropriate kernel in np, we need to coerce it to a factor here
tmp_data_all[,"Z2"] <- factor(tmp_data_all[,"Z2"], levels = c(0,1), ordered = FALSE)

cov_all <- tmp_data_all[, c("X1", "X2", "X3")]
mod_all <- tmp_data_all[,c("Z1", "Z2")]
...
lm_time <- system.time(lm_fit <- lm(Y ~ X1 + X2 + X3, data = tmp_data_train))["elapsed"]
...
save_list <- paste0(c("beta_mse_train_", "beta_coverage_train_", "beta_int_train_", 
                      ...
                      "timing_"), rep)
save(list = save_list, file = paste0("results/results_p3R2_", rep, ".RData"))
