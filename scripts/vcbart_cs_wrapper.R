# wrapper for VCBART used for panel data simulations with CS error structure
#library(VCBARTdev)
library(VCBART)
vcbart_cs_fixed_rho_wrapper <- function(Y_train, 
...
vcbart_cs_adapt_rho_wrapper <- function(Y_train, 
                                        X_train, Z_train, n_train,
                                        X_test, Z_test, n_test,
                                        cutpoints,
                                        error_structure,
                                        ...
                                        burn, nd, verbose, print_every)
{
  ...
  ystar_summary <- summarize_posterior_predictive(chain1, chain2, burn = burn)
  beta_support <- get_beta_support(chain1, chain2, burn, max_cutoff = 10)
  rho_samples <- c(chain1$rho_samples[-(1:burn)], chain2$rho_samples[-(1:burn)])
