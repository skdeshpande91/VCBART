...
source("scripts/honestRpart.R") # this is to compare with Zhou & Hooker (2019)
source("scripts/lgd.R") # this is to compare with Zhou & Hooker (2019)

btvcm <- function(Y_train, X_train, Z_train, X_test, Z_test,
  ...
  if(!is.null(tvcm_fit)){
    ...
    if(B == 0){
      ...
      results <- list(train = list(ystar = ystar_summary_train),
                      ...
                      time = time_train + beta_time + tmp_time)      
    } else{
      ######
      # Do the bootstrap now
      ######
      boot_beta <- array(dim = c(n_train + n_test, p + 1, B))
          ...
          system.time(
            {
              for(b in 1:B){
                ...
                boot_fit <- lgd(y=Y_train[boot_train], x=X_train[boot_train,], z=Z_train, model=model)
                
                boot_beta[1:n_train,,b] <- boot_fit$beta
                
                tmp_X <- matrix(0, nrow = n_train + n_test, ncol = ncol(X_train))
                tmp_int <- lgd.predict(tmp_X, rbind(Z_train, Z_test), boot_fit)
                
                boot_beta[,1,b] <- tmp_int
                for(j in 1:p){
                  tmp_X <- matrix(0, nrow = n_train + n_test, ncol = p)
                  tmp_X[,j] <- 1
      ...
      if(!is.null(boot_time)){
        ...
        beta_summary_train <- array(dim = c(n_train, 4, 1 + p), dimnames = list(c(), c("MEAN", "SD", "L95", "U95"), c()))
        beta_summary_test <- array(dim = c(n_test, 4, 1 + p), dimnames = list(c(), c("MEAN", "SD", "L95", "U95"), c()))
        ...
        beta_summary_test[,"SD",] <- boot_beta_se[(1 + n_train):(n_train + n_test),]
