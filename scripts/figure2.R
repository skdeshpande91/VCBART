...
beta_mse_mean <- apply(beta_mse_test_sigma2, FUN = mean, MARGIN = c(1,3))
beta_cov_mean <- apply(beta_cov_test_sigma2, FUN = mean, MARGIN = c(1,3))
beta_int_mean <- apply(beta_int_test_sigma2, FUN = mean, MARGIN = c(1,3))

png("figures/p5R20_performance.png", width = 8, height = 2.2, units = "in", res = 400)
...
text(x = 7.5, y = 1, labels = expression("VCBART"), cex = 0.95)
text(x = 12.5, y = 2, labels = expression("lm"), cex = 0.95)
text(x = 1.5, y = 3, labels = expression("KS"), cex = 0.95)
...
text(x = 0.6, y = 1, labels = expression("VCBART"), cex = 0.95)
...
text(x = 0.85, y = 5, labels = expression("BTVCM"), cex = 0.95)
abline(v = 0.95, lty = 2, col = 'red')


boxplot.matrix(ystar_rmse_test_sigma2[method_names,], use.cols = FALSE, horizontal = TRUE,
               main = expression("Predictive performance"), xlab = "", yaxt = "n",
               pch = 16, cex = 0.5, medlwd = 0.5, at = 1:8)
mtext(text = "RMSE\n(c)", side = 1, line = 3, cex = 1.25 * par('cex'))
text(x = 6, y = 1, labels = expression("VCBART"), cex = 0.95)
...
boxplot.matrix(ystar_cov_test_sigma2[method_names,], use.cols = FALSE, horizontal = TRUE,
...
text(x = 0.75, y = 2, labels = expression("lm"), cex = 0.95)
