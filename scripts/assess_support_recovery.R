...
assess_support_recovery <- function(varcount_mean, varcount_prob, theta_mean, true_support, R){
  
  # Method 1 (theta_mean): r is in active set j iff E[theta_jr | ...] > critical threshold
  # Method 2 (varcount_mean): r is in active set j iff E[# times Z_j split in ensemble j | ...] > critical threshold
  # Method 3 (varcount_probs): r is in active set j iff P(Z_j split on at least once in ensemble j | ...) > critical threshold
  
  metrics <- c("tp", "fp", "fn", "tn", "sen", "spec", "prec", "acc", "mcc", "f1")
  
  recovery_theta_mean <- matrix(nrow = 101, ncol = length(metrics), dimnames = list(c(), metrics))
  ...
  varcount_mean_seq <- seq(0, 20, length = 101)
  ...
  for(i in 1:101){
    ...
    for(j in 1:6){
      theta_support[[j]] <- which(theta_mean[,j] > theta_seq[i])
    ...
    recovery_theta_mean[i,] <- get_confusion_matrix(true_support, theta_support, R)
  ...
  return(list("theta_mean" = recovery_theta_mean, 
              "varcount_mean" = recovery_varcount_mean,
