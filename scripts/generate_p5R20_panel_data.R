...
# Generate errors
# There are only 4 observations per subject
Sigma_eps <- matrix(rho_true, nrow = 4, ncol = 4) + diag(1 - rho_true, nrow = 4)
eps_all <- c()
for(i in 1:n){
  Sigma_eps <- matrix(rho_true, nrow = n_all[i], ncol = n_all[i]) + diag(1 - rho_true, nrow = n_all[i])
  eps_all <- c(eps_all, mvrnorm(n = 1, mu = rep(0, times = n_all[i]), Sigma = Sigma_eps))
}
...
save(N_all, n,  p, R, X_all, Z_all, Y_all, mu_all, beta_all, n_all,
     ...
     cutpoints, true_support, cov_all, mod_all, rho_true, file = "data/p5R20_rho75_data.RData")
