get_summary <- function(chain1, chain2, burn = 250){
  ...
  if(("theta_samples" %in% names(chain1)) && ("theta_samples" %in% names(chain2))){
    R <- dim(chain1$theta_samples)[1]
    theta_samples <- list("chain1" = chain1$theta_samples, "chain2" = chain2$theta_samples)
    theta_summary <- array(dim = c(R, 4, p), dimnames = list(paste0("theta_", 1:R), c("MEAN", "SD", "L95", "U95"), c()))
    var_inclusion <- matrix(nrow = R, ncol = p)
    for(k in 1:p){
      theta_summary[,"MEAN",k] <- apply(cbind(chain1$theta_samples[,k,-(1:burn)], chain2$theta_samples[,k,-(1:burn)]), 
                                        FUN = mean, MARGIN = 1, na.rm = TRUE)
      theta_summary[,"SD",k] <- apply(cbind(chain1$theta_samples[,k,-(1:burn)], chain2$theta_samples[,k,-(1:burn)]),
                                      FUN = sd, MARGIN = 1, na.rm = TRUE)
      theta_summary[,"L95",k] <- apply(cbind(chain1$theta_samples[,k,-(1:burn)], chain2$theta_samples[,k,-(1:burn)]),
                                       FUN = quantile, MARGIN = 1, probs = 0.025, na.rm = TRUE)
      theta_summary[,"U95",k] <- apply(cbind(chain1$theta_samples[,k,-(1:burn)], chain2$theta_samples[,k,-(1:burn)]),
                                       FUN = quantile, MARGIN = 1, probs = 0.975, na.rm = TRUE)
      var_inclusion[,k] <- apply(cbind(chain1$var_counts_samples[,k,-(1:burn)], chain2$var_counts_samples[,k,-(1:burn)]) > 0,
                                 FUN = mean, MARGIN = 1, na.rm = TRUE) # these are roughly marginal inclusion probabilities
    }
