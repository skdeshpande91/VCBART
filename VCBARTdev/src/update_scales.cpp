...
void update_sigma_ig_cs(double &sigma, double &rho_eps, sigma_prior_info &sigma_pi, data_info &di, RNG &gen)
{
  ...
  for(size_t i = 0; i < di.N; i++){
    ...
    scale_post += tmp_sum2 / (1.0 - rho_eps) - tmp_sum * tmp_sum * rho_eps/( (1.0 - rho_eps) * (1.0 + ((double) di.n[i] - 1) * rho_eps));
  }
  sigma = sqrt( (scale_post)/gen.chi_square(nu_post));
}
...
void update_sigma_ig_ar(double &sigma, double &rho_eps, sigma_prior_info &sigma_pi, data_info & di, RNG &gen)
{
  ...
  for(size_t i = 0; i < di.N; i++){
    ...
    for(size_t j = 0; j < di.n[i]; j++){
      if(j == 0){
        scale_post += 1.0/(1 - rho_eps * rho_eps) * di.rf[j + di.start_index[i]] * di.rf[j + di.start_index[i]];
        scale_post += -1.0 * rho_eps/(1 - rho_eps * rho_eps) * di.rf[j + di.start_index[i]] * di.rf[j + 1 + di.start_index[i]];
      } else if(j == di.n[i] - 1){
        scale_post += 1.0/(1.0 - rho_eps * rho_eps) * di.rf[j + di.start_index[i]] * di.rf[j + di.start_index[i]];
        scale_post += -1.0 * rho_eps/(1.0 - rho_eps * rho_eps) * di.rf[j + di.start_index[i]] * di.rf[j - 1 + di.start_index[i]];
      } else{
        scale_post += (1 + rho_eps * rho_eps)/(1.0 - rho_eps * rho_eps) * di.rf[j + di.start_index[i]] * di.rf[j + di.start_index[i]];
        scale_post += -1.0 * rho_eps /(1.0 - rho_eps * rho_eps) * di.rf[j + di.start_index[i]] * di.rf[j - 1 + di.start_index[i]];
        scale_post += -1.0 * rho_eps/(1.0 - rho_eps * rho_eps) * di.rf[j + di.start_index[i]] * di.rf[j + 1 + di.start_index[i]];
      }
    }
  ...
  for(size_t i = 0; i < di.N; i++){
    ...
    for(size_t j = 0; j < di.n[i]; j++){
      for(size_t jj = j; jj < di.n[i]; jj++){
        Sigma(j,jj) = pow(rho_eps, jj-j);
        Sigma(jj,j) = pow(rho_eps, jj-j);
      }
